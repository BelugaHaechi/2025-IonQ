...
def run_challenge(graph_num: int=4, balanced: bool=True, dfs: bool=True, \
                  n_steps: int=40, lr: float=0.5, conv_epsilon: float=0.001):
    ...
    scores = (
        final_score(graph,XS_brut,counts,shots,ansatz,'base'),
        final_score(graph,XS_brut,counts,shots,ansatz,'balanced'),
        final_score(graph,XS_brut,counts,shots,ansatz,'connected')
    )


    print("\nSummary of Final Score\n")

    print("Base score: " + str(scores[0]))
    print("Balanced score: " + str(scores[1]))
    print("Connected score: " + str(scores[2]))
    ...
    return qit_evolver.energies, scores
